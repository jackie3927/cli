...
var ImMessagesEdit = common.Shortcut{
	...
	Description: "Edit a message's content (text/post, including the attachment zone); user/bot; PUT /open-apis/im/v1/messages/:message_id",
	Risk:        "write",
	...
	Validate: func(ctx context.Context, runtime *common.RuntimeContext) error {
		...
		if msg := validateEditContentFlags(text, markdown, content, attachments); msg != "" {
	...
	Execute: func(ctx context.Context, runtime *common.RuntimeContext) error {
		...
		if clearAttachments {
			...
			content = cleared
		} else if items, err := parseAttachments(attachments); err == nil && len(items) > 0 {
			msgType = "post"
			if content == "" {
				content = `{"zh_cn":{"content":[]}}`
			}
...
// only an attachment zone is allowed.
func validateEditContentFlags(text, markdown, content string, attachments []attachmentItem) string {
	contentFlags := 0
	if text != "" {
		contentFlags++
	}
	...
	if contentFlags == 0 && len(attachments) == 0 {
		return "specify --content <json>, --text <plain text>, --markdown <markdown text>, or --set-attachments <file_key>"
	}
	return ""
}

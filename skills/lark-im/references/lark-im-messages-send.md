...
| `post` | `{"zh_cn":{"title":"Title","content":[[{"tag":"text","text":"Body"}]]},"files":[{"key":"file_xxx","name":"report.pdf"}]}` — the top-level `files` array is the attachment zone; each entry carries a file/folder `key`, optional `name`, and optional `is_folder` |
